...
3. イベントを受け取ったら **150 ms のデバウンス**を挟んで `session.snapshot` を取り直す。イベントの差分を自前で適用するより単純で、状態が実体とずれない
4. ただしエージェントが出力している間 `pane.updated` は絶え間なく飛ぶため、デバウンスだけでは取得が止まらない。再取得は **最短 1 秒間隔**で頭打ちにする
5. スナップショットが更新されたら購読中のアクションへ通知する。ただし**キーの見た目に効く部分（エージェントの種別・状態・タイトル・並び、ワークスペース名、フォーカス）が変わっていなければ通知しない**。ペインの `revision` しか変わっていない更新で再描画しないため
...
herdr が検出する種別は 19 種類あり、全部に絵を用意しても見分けがつかない。よく使うものだけ形で表し、残りは頭 2 文字の字面で示す（`claude` と `cline` のように頭 1 文字では被るため 2 文字）。

| 種別 | グリフ |
| --- | --- |
| `claude` | 8 方向の光条（Claude Code が端末に出す `✳` に合わせる） |
| `codex` | 中心と 6 方向の粒からなる花形 |
| `gemini` | 4 方向に尖った星 |
| `cursor` | 矢印カーソル |
| その他 | 頭 2 文字の字面。種別が検出できていなければ `?` |

`send-keys` はプリセットごとに静的アイコンを差し替える（承認 = チェック、拒否 = バツ、はい = `Y`、いいえ = `N`、自由入力 = キーキャップ）。`prompt` / `interrupt` は固定のアイコンを使う。いずれもタイトルにユーザー設定のラベルを出し、押下に失敗したら `showAlert()`、成功したら `showOk()` を出す。
...
| ストア | フェイクタイマー。デバウンスと最短間隔、見た目が変わらないときは通知しないこと、購読者への通知、オフライン遷移と復帰 |
